...
client.on('message', message => {
  ...
  if (isReady) {
    for (var keyname in quotes) {
      if (msg.startsWith(`${prefix} ${keyname}`) && typeof quotes[keyname] === 'object') {
        var options = Object.keys(quotes[keyname]).length
        var randomNumber = (Math.floor(Math.random() * options) + 1).toString()
      ...
      } else if (msg.startsWith(`${prefix} ${keyname}`)) { // A one-to-one voice command match
  ...
  if (msg.startsWith(`${prefix} help`)) {
    var commandString = `\`\`\``
    message.channel.send(`Who says I can't handle my drink?\nHere are my voice commands:\n_Always_ prefix with **_yas[space][command]**\n`)
